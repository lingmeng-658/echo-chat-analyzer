...
def test_reply_message_excludes_auto_mention_from_authored_text() -> None:
    """Regression: authored text must not include the auto-@ of the replied user.

    Real QCE structure for reply + auto-mention + authored text:

        content.text = "[回复消息]@被回复者昵称 当前作者正文"
        content.elements = [
            {"type": "reply", "data": {"senderName": "被回复者昵称", ...}},
            {"type": "at", "data": {"name": "被回复者昵称", "atType": 2}},
            {"type": "text", "data": {"text": " 当前作者正文"}},
        ]

    The auto-@ element (type=at) is NOT authored text. Only the text element
    data.text is authored.

    This test constructs the exact real QCE structure with fictional data
    containing Unicode emoji (\U0001f40a in the mention name, \U0001f602 in
    the authored text) and asserts message.contents contains only the authored
    text.

    Current failure root cause:
    _ordered_content_parts() iterates elements. The reply element and the at
    element produce no TextContent or ExpressionContent. Only the text element
    produces TextContent(text=" 依旧\U0001f602"). has_expression is False,
    so the function returns None at the "if not has_expression" guard.
    _build_rich_contents then falls back to content.text, which includes the
    auto-@ prefix.
    """
    raw_message = _qce_message(
        message_type="reply",
        message_id="fake-reply-auto-mention",
        text="[回复消息]@Fictional Bob\U0001f40a 依旧\U0001f602",
    )
    raw_message["content"]["elements"] = [
        # 1. reply element
        {
            ...
            "data": {
                ...
                "senderUin": "2002",
                "senderName": "Fictional Bob\U0001f40a",
                "content": "被回复内容",
                "timestamp": 1750000000,
            },
        },
        # 2. auto-@ element (generated by QQ, not authored)
        {
            "type": "at",
            "data": {
    ...
    # Core assertion: authored text must be ONLY the text element content.
    # The auto-@ "Fictional Bob\U0001f40a" must NOT appear in contents.
